...
Learn how requests are routed to agents, how naming works, and patterns for organizing your agents with custom routing.
...
const agent = useAgent({
  agent: "UserProfile",
  name: `user-${userId}` // for example, "user-abc123"
});
```
...
const agent = useAgent({
  agent: "ChatRoom",
  name: roomId // for example, "general" or "room-42"
});
```
...
- The instance name is determined server-side (for example, from auth or session)
...
| Instance from auth or session | `basePath` + `getAgentByName` + `fetch` |
...
| `basePath` | `string` | Full URL path - bypasses agent or name URL construction |
...
- [Calling Agents](/agents/api-reference/calling-agents/) — Learn how to call agents from Workers
- [WebSockets](/agents/api-reference/websockets/) — Real-time connections to agents
