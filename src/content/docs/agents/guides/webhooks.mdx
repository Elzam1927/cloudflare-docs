...
import { FileTree, TypeScriptExample } from "~/components";
...
| **CI/CD Pipeline Agent** | React to build or deploy events, notify on failures, track deployment history |
| **Linear or Jira Tracker** | Auto-triage issues, assign based on content, track resolution times |
...
| Use Case | Description |
| --- | --- |
| **Twilio SMS or Voice** | Conversational agents triggered by inbound messages or calls |
| **Slack Bot** | Respond to slash commands, button clicks, and interactive messages |
| **Email Tracking** | SendGrid or Mailgun delivery events, bounce handling, engagement analytics |
...
3. **Respond quickly** — Return 200 or 202 within seconds to avoid retries
...
- [Routing](/agents/api-reference/routing/) — Custom routing patterns
- [State Management](/agents/api-reference/store-and-sync-state/) — Managing agent state
- [Callable Methods](/agents/api-reference/callable-methods/) — Client-side method calls
